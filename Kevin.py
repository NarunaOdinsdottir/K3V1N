...
def raetsel():
    frage, loesung = random.choice([
        ("Ich habe viele Blätter, aber bin kein Baum. Was bin ich?", "buch"),
        ("Je mehr du wegnimmst, desto größer wird es. Was ist das?", "loch"),
        ("Ich geh vorwärts, niemals zurück. Was bin ich?", "uhr")
    ])
    antwort = input(f"Rätsel: {frage} ").strip().lower()
    if antwort == loesung:
        print("Richtig! *reicht zwei Kekse*")
    else:
        print(f"Falsch! Die richtige Antwort war: {loesung}. Aber du bekommst trotzdem einen Keks!")
